...
double INa_IKur_Drug_Updated_CNZ_ODE(double dt, double Istim, SingleCellPara & het, double *state) {
	...
	// IKr
	IKr = het.GKr * Cm * CRN_gkr * xr * (V - Ek) / (1 + exp((V + 15) / 22.4));// 90% haibo
	...
	//Ito
	Ito = 1.05 * het.Gto * Cm * MAL_gto * itr * its * (V - Ek);
	...
	IKur = het.GKur * Cm * CNZ_gkur * (4.5128 + 1.899769 / (1.0 + exp((V - 20.5232) / (-8.26597)))) * (1 - IKur_BO - IKur_BC) * CNZ_a * CNZ_i * (V - Ek);
	...
	inf = 1 / (1 + exp(-(V  + 5.52) / (8.6))); // change to simple version, haibo.  removed IKur mutations here.
	...
	//Iab
	Iab = 0.* Cm * 0.0003879 * (V + 69.6) / (1 - 0.8377 * exp((V + 49.06) / 1056));

	//Ibk
	IbK =  0.0;//Cm * CRN_gbk * (V - Ek);

	//IbCa
	IbCa = 1.0 * Cm * CRN_gbca * (V - Eca); // increased background IbCa, haibo
	// IbNa
	IbNa = 1.7 * Cm * CRN_gbna * (V - Ena);  // increase. haibo

	//ICap
	ICap = 1.26 * het.GCap * Cm * CRN_icapbar * Cass / (Cass + CRN_kmcap);  // change, haibo
	// ICap = 1.1 * het.GCap * Cm * CRN_icapbar * Cass / (Cass + CRN_kmcap);  // change, haibo



	//INaCa, times 1.5, Haibo
	INaCa = 1.4 * het.GNaCa * Cm * CRN_knacalr / (pow(CRN_kmnalr, 3.0) + pow(CRN_nac, 3.0)) / (CRN_kmcalr + CRN_cac) /
	...
	// verison 3.0
	INaK = 1.28 * Cm * CRN_inakbar * fnak * CRN_kc / (CRN_kc + CRN_kmko) / (1 + pow(/*CRN_kmnai*/10. / nai, 4)); // increase, Haibo.
...
void INa_IKur_Drug_Updated_CNZ_ODE_Initialise(double *state, int celltype) {
	state[0]  = -77.13255836;
